...
bool isPrintFinished = false; //REPRINT IMPLEMENT
...
#include "xflash.h"
  ...
  static void lcd_main_menu()
  {
	...
	if ( (!PRINTER_ACTIVE) && isPrintFinished) // REPRINT IMPLEMENT
  	{
...
#define MENU_ITEM_EDIT_advance_K() do { if (lcd_advance_K()) return; } while (0)
  ...
  static bool selftest_irsensor()
  {
    class TempBackup
    {
      public:
        ...
        ~TempBackup() {
          setTargetHotend(m_temp, m_extruder);
        }
      private:
        float m_temp;
        uint8_t m_extruder;
  ...
  static void lcd_selftest_measure_fans(uint8_t delay, uint8_t col, uint8_t row) {
    // spin-up delay
    static char symbols[] = {'-', '|'};
    static_assert(1000 / sizeof(symbols), "-"); // DON'T TRUST THIS, ITS A BANDAID static_assert(1000 / sizeof(symbols) * sizeof(symbols) == 1000);
    while (delay--) {
  ...
  void reprint_from_eeprom() { //REPRINT IMPLEMENT
	char cmd[30];
	char filename[13];
	uint8_t depth = 0;
	char dir_name[9];
